...
_EVALUATION_DESIGN_PAIRED_2X2 = "paired_2x2"
_EVALUATION_DESIGN_DATASET_WIDE = "dataset_wide"
_VALID_EVALUATION_DESIGNS = (
    _EVALUATION_DESIGN_PAIRED_2X2,
    _EVALUATION_DESIGN_DATASET_WIDE,
)
...
def _normalize_evaluation_design(value: object) -> str:
    ...
    if normalized not in _VALID_EVALUATION_DESIGNS:
        raise ValueError(
            f"evaluation_design must be one of {list(_VALID_EVALUATION_DESIGNS)}"
        )
    return normalized
...
class BaseRobustnessIndex(ABC):
    ...
    def _score_from_neighbors(
        ...
        **kwargs: float,
        ...
        """Vectorized SO/OS scoring for all samples at a single k.

        Returns (pooled_score, sample_scores, informative_mask, undefined_type).
        """
        n = len(labels)
        target_k = int(k)

        # Cap effective k per sample
        eff_k = np.minimum(valid_counts, target_k)  # (n,)

        # Build validity mask: which (sample, neighbor) slots are usable
        col_indices = np.arange(min(target_k, neigh_idx.shape[1]))[np.newaxis, :]  # (1, kmax)
        slot_valid = (col_indices < eff_k[:, np.newaxis]) & (neigh_idx[:, :target_k] >= 0)  # (n, kmax)

        # Slice neighbor arrays to target_k columns
        idx = neigh_idx[:, :target_k]  # (n, kmax)
        dist = neigh_dist[:, :target_k]  # (n, kmax)

        # Clamp invalid indices to 0 for safe gather (masked out later)
        safe_idx = np.where(slot_valid, idx, 0)

        # Gather neighbor labels and centers
        neigh_labels = labels[safe_idx]  # (n, kmax)
        neigh_centers = centers[safe_idx]  # (n, kmax)

        # Compare with query sample
        same_label = neigh_labels == labels[:, np.newaxis]  # (n, kmax)
        same_center = neigh_centers == centers[:, np.newaxis]  # (n, kmax)
        ...
        # Compute weights (works element-wise on 2-D arrays)
        weights = cls._weights(dist, **kwargs)  # (n, kmax)

        # Weighted SO/OS per sample
        so_per_sample = np.where(so_mask, weights, 0.0).sum(axis=1)  # (n,)
        os_per_sample = np.where(os_mask, weights, 0.0).sum(axis=1)  # (n,)
        ...
        # Undefined type classification for non-informative samples
    ...
    def _score_all_k_from_neighbors(
        ...
        **kwargs: float,
        ...
        """Vectorized SO/OS scoring for all samples at multiple k values via cumsum.

        Builds weighted SO/OS matrices once, then uses cumulative sums to derive
        results for every requested k in a single pass.

        Returns per-k: (pooled_score, sample_scores, informative, undefined_type,
                         so_per_sample, os_per_sample).
        """
        n = len(labels)
        ...
        # Weighted per-neighbor SO/OS values, zeroed where invalid
        so_weighted = np.where(so_mask, weights, 0.0)  # (n, actual_cols)
        os_weighted = np.where(os_mask, weights, 0.0)

        # Cumulative sums along the neighbor axis
        so_cum = np.cumsum(so_weighted, axis=1)  # (n, actual_cols)
        os_cum = np.cumsum(os_weighted, axis=1)

        # Also cumsum SS/OO counts for undefined subtyping
        ...
        for k in k_values:
            ki = min(int(k), actual_cols) - 1  # column index (0-based)
            if ki < 0:
                out[int(k)] = (
                    0.5,
    ...
    def _select_subset_k(
        ...
        *,
        ...
        """Select optimal k by averaging within-subset k-NN balanced accuracy.

        Matches PathoROB: run k-NN classification inside each 2x2 subset,
        average balanced accuracies across subsets at each k, pick argmax.
        """
        ...
        for subset in subsets:
            ...
            sub_candidates = [k for k in k_candidates if k < len(prepared.source_indices)]
        ...
        # Average across subsets at each k (equal weight per subset)
    ...
    def _compute(
        ...
        *,
        ...
        evaluation_design: str = _EVALUATION_DESIGN_PAIRED_2X2,
        ...
        **kwargs: float,
        ...
        if evaluation_design == _EVALUATION_DESIGN_PAIRED_2X2:
        ...
        if evaluation_design == _EVALUATION_DESIGN_DATASET_WIDE:
        ...
        **kwargs: float,
        ...
        for k in candidates:
            ...
            undefined_breakdown = cls._compute_undefined_breakdown(
                ...
                ss_undefined=ss_undefined,
                oo_undefined=oo_undefined,
    ...
    def _compute_dataset_wide(
        ...
        **kwargs: float,
        ...
        return RobustnessResult(
            ...
            oo_dominated_undefined_frac=oo_dominated_undefined_frac,
            mixed_undefined_frac=mixed_undefined_frac,
            evaluation_design=_EVALUATION_DESIGN_DATASET_WIDE,
            evaluation_unit="sample",
        )
    ...
    def _score_subsets_by_k(
        ...
        **kwargs: float,
        ...
        # Per-subset pooled scores (for diagnostics: std, n_pairs)
        per_k_pair_values: dict[int, list[float]] = {int(k): [] for k in candidates}
        # Accumulated raw SO/OS across all subsets (PathoROB-compatible pooling:
        # a sample appearing in N subsets contributes N times)
        ...
        for subset in subsets:
            ...
            # Score all k values in one vectorized pass per subset
            all_k_results = cls._score_all_k_from_neighbors(
                labels=prepared.labels,
                centers=prepared.centers,
                neigh_idx=neigh_idx,
            ...
            for k in candidates:
                pair_value, per_sample, informative_mask, undefined_type, so_arr, os_arr = (
                    all_k_results[int(k)]
                )
                per_k_pair_values[int(k)].append(float(pair_value))
                per_k_occurrence_total[int(k)] += int(len(per_sample))

                # Accumulate raw SO/OS (each subset contribution adds, matching PathoROB)
        ...
        for k in candidates:
            ...
            # Pooled value from accumulated SO/OS counts (PathoROB-compatible)
