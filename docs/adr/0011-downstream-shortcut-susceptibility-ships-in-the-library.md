...
reductions (`apd`, `nipd`). This narrows ADR-0002: the library is no longer
representation-metrics-only.

## Why

nIPD is a named metric contribution of the paper, and the manuscript tells readers
...
The reduction alone was the obvious lean compromise — export
`nipd(accuracies, cramers_v, chance)` as a pure function and leave the protocol in
`scripts/`. It was rejected because the
...
- **`croma.downstream`** — the probe protocol plus `apd` and `nipd`. Consumes
...
- ADR-0002's "the contribution is the metrics (RI, MaRI, CRoMa)" now reads
  "(RI, MaRI, CRoMa, nIPD)". The `scripts/`-is-not-a-product principle is unchanged.
...
- nIPD requires mean baseline balanced accuracy to exceed chance, because otherwise its
  normalizing skill is not positive. It carries no additional weak-skill gate.
- ADR-0018 defines the nIPD estimand and records the clean replacement of nAPD.
