...
def test_committed_metadata_marks_the_four_pathorob_wsis() -> None:
    ...
    assert r"UNI2-h~\cite{uni2h} & DINOv2 & n/d & $350$k$^{\dagger}$ / n/d" in body
    assert r"CONCHv1.5~\cite{titan} & vision--language & n/d & $100$k$^{\dagger}$ / n/d" in body
    assert r"CONCH~\cite{conch} & vision--language & $86$M & $21.4$k$^{\dagger}$ / $16$M" in body
    assert r"Prost40M~\cite{grisi2026bcr} & DINO & ${\sim}22$M & $2$k$^{\dagger}$ / ${\sim}40$M" in body
