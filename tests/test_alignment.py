...
def _write_repeated_subset_manifest(path: Path) -> pd.DataFrame:
    ...
    payload = ["sample_id,image_path,label,medical_center,slide_id,subset"]
    for subset in ("pair1", "pair2"):
        for sample_id, image_path, label, center, slide_id in rows:
            payload.append(",".join([sample_id, image_path, label, center, slide_id, subset]))
    path.write_text("\n".join(payload) + "\n", encoding="utf-8")
...
def test_expand_features_to_manifest_repeats_deduplicated_rows(tmp_path: Path) -> None:
    ...
    manifest_df = _write_repeated_subset_manifest(manifest_path)
    manifest = load_manifest(str(manifest_path))
    embedding_manifest, _ = build_embedding_source_manifest(manifest_df)
    features = np.asarray(
        [
            [1.0, 0.0],
            [2.0, 0.0],
