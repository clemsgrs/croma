...
def test_metrics_are_scoped_by_protocol_then_benchmark() -> None:
    assert layout.metrics_dir("median-k", "camelyon") == (
        layout.OUTPUT_ROOT / "metrics" / "median-k" / "camelyon"
    )
    assert layout.results_dir("k-star", "camelyon").parts[-2:] == ("camelyon", "results")


def test_unknown_protocol_is_rejected() -> None:
    with pytest.raises(ValueError, match="unknown protocol"):
        layout.metrics_dir("median_k", "camelyon")
...
def test_benchmarks_share_tilesets_so_embeddings_are_never_duplicated() -> None:
    ...
    assert set(benchmarks.TILESETS["pathorob-camelyon"]) == {"camelyon", "camelyon-full"}
