...
(e.g. Virchow2 CCMR 0.21 in figures vs 0.20 in the table). This script renders
...
paper on the faithful run. The four experiment-script figures (ccmr_rank,
...
METRICS_FIGS = {
    "ccmr_ltm_bars": plotting.plot_ccmr_ltm_bars,
    "ccmr_vs_mari_scatter": plotting.plot_ccmr_vs_mari_scatter,
...
def main() -> int:
    ...
    m_sweep = json.loads((SRC / "ccmr_m_sweep_metrics.json").read_text())
    ...
    for name, fn in KSWEEP_FIGS.items():
        ...
        rendered.append(name)
    plotting.plot_ccmr_m_sweep(rows=m_sweep, out_path=TMP / "ccmr_m_sweep.png")
    rendered.append("ccmr_m_sweep")
