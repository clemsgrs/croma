...
This is the raw material the APD scalar reduces:

    APD = mean_{V>0} ( acc(V) / acc(0) ) - 1
...
from loaders import REPO, ranked, training_correlations  # noqa: E402
...
def _mean_and_ci(dataset, model, regime):
    ...
    accuracies = np.asarray(raw[f"{regime}_test_accuracies"], dtype=float) * 100.0
...
def _draw_panel(ax, dataset, regime, *, ylim, correlations):
    for model in _roster(dataset):
        means, half_width = _mean_and_ci(dataset, model, regime)
        ax.errorbar(
